...
private:
    ...
    string t_lua_generator::full_type_name_of_struct(t_struct* tstruct);
...
void t_lua_generator::init_generator() {
    ...
    string f_consts_name = outdir + program_->get_name() + "_" + "constants.lua";
    f_consts_.open(f_consts_name.c_str());
    string f_types_name = outdir + program_->get_name() + "_" + "ttypes.lua";
...
void t_lua_generator::generate_process_function(ostream& out,
    ...
    out << indent() << "self.handler." << fn_name
        << "(self.handler";
    // Print arguments
...
void t_lua_generator::generate_deserialize_struct(ostream& out,
    ...
    bool local,
    string prefix) {
    indent(out) << (local ? "local " : "") << prefix << " = " << full_type_name_of_struct(tstruct) << ":new{}"
...
string t_lua_generator::full_type_name_of_struct(t_struct* tstruct) {
    ...
    if (program != program_ &&
        !program->get_namespace("lua").empty() &&
        program_->get_namespace("lua") != program->get_namespace("lua")) {
        return program->get_namespace("lua") + "." + tstruct->get_name();
    }
    else {
